...
- **FaqSection.vue** — FAQ en accordéon (contenu réel : questions/réponses éditables via Sanity, avec valeurs par défaut dans le composant). Rendu responsive : titre/sous-titre rétrécis, sous-titre qui avait un `margin-left: 310px` débordant corrigé, padding vertical augmenté, cards centrées avec inset supplémentaire.
...
- **FormSection.vue** — formulaire de contact (prénom, nom, email, téléphone optionnel, sujet en dropdown, message, case consentement, bouton d'envoi). Désormais fonctionnel : champs liés avec `v-model`, envoi à Web3Forms au submit, message de succès affiché après envoi, message d'erreur en cas d'échec, bouton désactivé pendant l'envoi. L'objet "Candidature / collaboration" est routé vers la clé RH (`web3formsKeyRh`) si elle est renseignée, tous les autres objets vers la clé générale (`web3formsKey`). Rendu responsive : champs prénom/nom et email/téléphone en 1 colonne sous 768px, bouton pleine largeur sous 480px.

  **Envoi des emails via Web3Forms** (https://web3forms.com — gratuit, fonctionne en hébergement statique sans serveur). L'envoi se fait côté client via `POST https://api.web3forms.com/submit`. Les clés d'accès sont PUBLIQUES par design (elles identifient seulement la boîte de réception, aucun secret) et se définissent via les variables `NUXT_PUBLIC_WEB3FORMS_KEY` et `NUXT_PUBLIC_WEB3FORMS_KEY_RH`.

  **Ce que Marie doit faire avant la mise en prod :**
  1. Aller sur [web3forms.com](https://web3forms.com), saisir l'adresse générale du cabinet → récupérer la clé d'accès reçue par mail (idem avec l'adresse RH pour la 2ᵉ clé).
  2. Renseigner les variables dans Vercel (Settings → Environment Variables), puis en local dans `.env` :
     - `NUXT_PUBLIC_WEB3FORMS_KEY` → clé de la boîte générale
     - `NUXT_PUBLIC_WEB3FORMS_KEY_RH` → clé de la boîte RH (optionnelle)
  3. Redéployer sur Vercel pour que les nouvelles variables soient prises en compte.
...
- **HistorySection.vue** *(nouveau)* — section "Notre histoire" juste après le hero : frise chronologique verticale alternée (gauche/droite), titre en pill teal (même style que "Nos chiffres clés"), 5 jalons avec années réelles (1985, 1998, 2010, 2018, 2024). Anime au scroll (slide-in gauche/droite via `v-reveal`). Avait déjà un bon point de bascule responsive à 700px (frise → liste verticale alignée à gauche) ; complété *(branche `page-qui-sommes-nous`)* avec le rétrécissement du titre (pill en `nowrap`) et un padding latéral cohérent sous 768px/480px.
